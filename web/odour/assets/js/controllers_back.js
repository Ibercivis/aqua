...
angular.module('odourCollectApp.controllers', [])
  ...
  .controller('MapCtrl', function ($scope, $rootScope, $timeout, $q, leafletData, gettextCatalog, REPORT_EVENTS, Position, ReportsService, AuthService, Dialogs, leafletMarkerEvents) {
    $scope.filters = {
      type: '',
    }
    ...
    var greyIcon = angular.copy(baseIcon);

    goodIcon.iconUrl = 'assets/img/good_water.png';
    badIcon.iconUrl = 'assets/img/bad_water.png';
    midIcon.iconUrl = 'assets/img/mid_water.png';
    greyIcon.iconUrl = 'assets/img/grey_water.png';
    ...
    addReportControl.setPosition('topleft');
    addReportControl.onAdd = function () {
      var container = L.DomUtil.create('div', '');
      container.innerHTML = gettextCatalog.getString('');

      container.style.padding = '5px';
      container.style.background =  'url(assets/img/plus.png)';
  ...
  $scope.$on('leafletDirectiveMarker.map.dragend',function(event,args) {
        Position.set(args.leafletEvent.target._latlng.lat, 
                     args.leafletEvent.target._latlng.lng, $scope.center.zoom);
    ...
    $scope.$on('leafletDirectiveMarker.map.click',function(event,args) {
        if(args.modelName==0){      /* Hemos pulsado en la gota gris*/
      ...
      overlays: {
        odours: {
          ...
          layerOptions: {
            ...
            disableClusteringAtZoom: 16, // On max zoom, markers will not be clustered
          },
          visible: true
        }
      }
    ...
    function addCustomIcon(){
		  console.log("In add Custom Icon")
			var markers = {};
      var marker ={
	    	layer: 'odours',
      	lat  : Position.lat,
